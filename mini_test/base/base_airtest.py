import sys, os
cur_path = os.getcwd()
sys.path.append(cur_path[:cur_path.find('miniworld_uitest') + len('miniworld_uitest')])
import subprocess
...
class BaseAirtest(object):
    ...
    def snapshot_img(cls,filename,*recursive_path):
        filename = cls.get_snapshot_path(filename,*recursive_path)
        snapshot(filename)
    ...
    @handle_poco_timeout(retries=3, timeout=5)
    def poco_set_text(self,button,text):
        self.poco(button).set_text(text)

    @handle_poco_timeout(retries=3, timeout=5)
    def poco_get_text(self,button):
        return self.poco(button).get_text()
    
    @handle_poco_timeout(retries=3, timeout=5)
    def poco_exists(self,button):
        return self.poco(button).exists()
