    ...
    for dev in devices:
        if (not run_all and results['tests'].get(dev) and
           results['tests'].get(dev).get('status') == 0):
            ...
            continue
        model = (connect_device("android:///" + dev)).adb.get_model()
        log_dir = get_log_dir(model, air) #打印各设备运行日志
        cmd = [
            "airtest",
            "run",
            air,
...
def clear_log_dir(air):
    """"
        清理log文件夹 test.air/log
        Remove folder test.air/log
    """
...
def get_log_dir(device, air):
    """"
        在 test.air/log/ 文件夹下创建每台设备的运行日志文件夹
        Create log folder based on device name under test.air/log/
    """
